...
const connectionString =
	process.env.DATABASE_URL || "postgresql://localhost:5432/todo_mcp";
...
export class TodoDatabase {
	...
	async getAllLists(): Promise<schema.SelectList[]> {
		return await db
			.select()
			.from(schema.listsTable)
			.where(isNull(schema.listsTable.archivedAt));
	}

	async updateList(
		id: number,
		data: schema.UpdateList,
	): Promise<schema.SelectList | undefined> {
		const [list] = await db
	...
	async getTasksByList(listId: number): Promise<schema.SelectTask[]> {
		return await db
			.select()
			.from(schema.tasksTable)
			.where(eq(schema.tasksTable.listId, listId));
	}
	...
	async getActiveTasks(): Promise<schema.SelectTask[]> {
		return await db
			.select()
			.from(schema.tasksTable)
			.where(isNull(schema.tasksTable.completedAt));
	}
	...
	): Promise<schema.SelectTask | undefined> {
		const [task] = await db
